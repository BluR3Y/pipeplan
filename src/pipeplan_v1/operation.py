...
class Operation(ABC):
    ...
    @classmethod
    def register_operation(cls, fn_name: str):
        def decorator(fn: Callable):
            if fn_name in cls._function_registry:
                raise ValueError(f"Operation '{fn_name}' already exists in {cls.__name__}")
            cls._function_registry[fn_name] = fn
            return fn
        return decorator
    
    @classmethod
    def get_operation(cls, fn_name: str) -> Callable:
        ...
        if fn_name not in cls._function_registry:
            raise KeyError(f"Unknown operation '{fn_name}' in {cls.__name__}")
        return cls._function_registry[fn_name]
